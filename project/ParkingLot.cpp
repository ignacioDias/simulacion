...
double ParkingLot::ta(double t) {
	return sigma;
}
void ParkingLot::dint(double t) {
	if (!vehicles.empty()) {
		vehicles.pop_front();
	}
...
void ParkingLot::dext(Event x, double t) {
	double u;
	double uniform;
	double time_car;
...
Event ParkingLot::lambda(double t) {
	if (!vehicles.empty()) {
